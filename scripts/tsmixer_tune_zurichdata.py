...
def objective(trial):
    ...
    ff_size = trial.suggest_categorical("ff_size", [16, 32, 64, 128, 256])
    dropout_rate = trial.suggest_float("dropout_rate", 0, 0.5)
    
    custom_early_stopping = CustomEarlyStopping(monitor="val_loss", patience=0, min_delta=0.001, mode='min')

    tsmixer_model = TSMixerModel(
        input_chunk_length=input_chunk_length,
        output_chunk_length=forecast_horizon,
        n_epochs=10,
        model_name='Opt_tsmixer',
    ...
    tsmixer_model.fit(train_transformed[:len(train_transformed)],
