...
tide_model.fit(train_transformed, past_covariates = past_covariates_transformed, future_covariates = future_covariates_transformed, val_series=validation_transformed, val_past_covariates = past_covariates_transformed, val_future_covariates = future_covariates_transformed, verbose=True)
print("Train time: %s seconds" % (time.time() - start_time))
tide_model.load_from_checkpoint(model_name='TiDE', best=True)
tide_model.save(f"models/tide_zurich_model_{input_chunk_length}_{forecast_horizon}.pkl")

tide_model = TiDEModel(input_chunk_length=input_chunk_length,output_chunk_length=forecast_horizon,n_epochs=10,model_name='TiDE',hidden_size=hidden_size,dropout=dropout_rate,use_reversible_instance_norm=True,add_encoders={'cyclic': {'future': ['day','week','month']}},likelihood=QuantileRegression(),pl_trainer_kwargs={"gradient_clip_val": 1, "callbacks": [EarlyStopping(monitor="val_loss",patience=3, min_delta=0.0001,mode='min')]},save_checkpoints=True,force_reset=True,random_state=42,)
tide_model.fit(train_transformed, future_covariates=future_covariates_transformed, val_series=validation_transformed, val_future_covariates = future_covariates_transformed, verbose=True)
print("Train time: %s seconds" % (time.time() - start_time))
tide_model.load_from_checkpoint(model_name='TiDE', best=True)
tide_model.save(f"models/tide_zurich_model_no_covs_{input_chunk_length}_{forecast_horizon}.pkl")

tide_model = TiDEModel(input_chunk_length=input_chunk_length,output_chunk_length=forecast_horizon,n_epochs=10,model_name='TiDE',hidden_size=hidden_size,dropout=dropout_rate,use_reversible_instance_norm=True,add_encoders={'cyclic': {'future': ['day','week','month']}},likelihood=QuantileRegression(),pl_trainer_kwargs={"gradient_clip_val": 1, "callbacks": [EarlyStopping(monitor="val_loss",patience=3, min_delta=0.0001,mode='min')]},save_checkpoints=True,force_reset=True,random_state=42,)
tide_model.fit(train_transformed, future_covariates=future_covariates_transformed.stack(past_covariates_transformed), val_series=validation_transformed, val_future_covariates = future_covariates_transformed.stack(past_covariates_transformed), verbose=True)
print("Train time: %s seconds" % (time.time() - start_time))
tide_model.load_from_checkpoint(model_name='TiDE', best=True)
tide_model.save(f"models/tide_zurich_model_forecast_covs_{input_chunk_length}_{forecast_horizon}.pkl")

tide_model = TiDEModel(input_chunk_length=input_chunk_length,output_chunk_length=forecast_horizon,n_epochs=10,model_name='TiDE',hidden_size=hidden_size,dropout=dropout_rate,use_reversible_instance_norm=True,add_encoders={'cyclic': {'future': ['day','week','month']}},likelihood=QuantileRegression(),pl_trainer_kwargs={"gradient_clip_val": 1, "callbacks": [EarlyStopping(monitor="val_loss",patience=3, min_delta=0.0001,mode='min')]},save_checkpoints=True,force_reset=True,random_state=42,)
tide_model.fit(train_transformed, future_covariates=future_covariates_transformed_no_holidays, val_series=validation_transformed, val_future_covariates = future_covariates_transformed_no_holidays, verbose=True)
print("Train time: %s seconds" % (time.time() - start_time))
tide_model.load_from_checkpoint(model_name='TiDE', best=True)
tide_model.save(f"models/tide_zurich_model_no_holidays_{input_chunk_length}_{forecast_horizon}.pkl")